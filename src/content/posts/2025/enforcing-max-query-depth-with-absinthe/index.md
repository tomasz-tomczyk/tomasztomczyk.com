...
cover: "./desk-at-sunset.png"
coverAlt: "A laptop showing code on a wooden desk by a window, looking out at a lake and mountains at sunset, with a mug and a stack of books on Elixir, Oban, PostgreSQL and distributed systems."
coverPosition: "center 40%"

# Demonstration of per-post theming (added while migrating — revert if unwanted).
# Every colour role is re-pointed together; leaving any text role on the light
# default would fail the build's contrast guard rather than ship unreadable.
theme:
  surface: "#14161A"
  raised: "#1C1F24"
  line: "#2A2E35"
  ...
  ink: "#E8E6E1"
  accent: "#4A9EFF"
  # Syntax roles sit on `raised`, so they move with it. Light-palette values
  # on this background are ~2.2:1 and the build refuses them.
  # Numbers are amber, not blue: this post's accent is blue, and two blues in
  # one code block read as one colour. Each post picks a set that separates
  # from its own accent.
  code-string: "#72D699"
...
```graphql title="A deeply nested query"
query IAmEvil {
  author(id: "abc") {
    posts {
      author {
...
```elixir title="Selection depth middleware"
