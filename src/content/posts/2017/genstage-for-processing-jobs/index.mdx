...
<ArchiveNote title="This article is from 2017.">

GenStage still works the way it is described here, but for a job pipeline you are probably better off with
[Broadway](https://github.com/dashbitco/broadway), which wraps this pattern and
handles batching, rate limiting and acknowledgement for you.

</ArchiveNote>

We use Elixir at <IconLabel label="uSwitch" /> to process user-submitted forms, sending the data to a 3rd party API and parsing the output saving the results to a database. The high-level outline of the Elixir process as a recursive loop looks like this:
