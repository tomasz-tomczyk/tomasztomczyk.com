...
<ArchiveNote title="This guide is from 2019.">

The pipeline below is pinned to Elixir 1.10 and to the GitLab CI syntax of the
time, so copying it as-is will not get you a working build today. The shape
still holds — the stages, the caching and the release steps are what I would
reach for again — but treat the YAML as a sketch rather than a template.
